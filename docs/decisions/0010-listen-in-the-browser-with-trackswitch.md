...
`scripts/build_demo.py` → `index.html` (player configs inline) plus one spectrogram strip per track. The audio is
never copied, cut, or re-encoded: `demo/artifacts` links to the artifacts directory, players reference each pack's
files where the experiment wrote them, and a gate refuses any pack that is no longer one level-matched set (all
tracks within 0.1 LU). A comparison that needs different material is a different pack, not a windowing option.
...
- **`sdedit`** — the instrument: every variant of every pack, one player per family, created only when its section
  is expanded; at most one player stays decoded at a time.
...
- Generated spectrograms stay out of git; page code, configs and the `demo/artifacts` link are committed. The audio
  has no second copy anywhere — the pages play the packs themselves.
