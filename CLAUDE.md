...
Portable across projects. Nothing here should be specific to one codebase — project knowledge lives in that
project's ADRs and its `operate-*` skill.
...
- **`.claude/skills/operate-<project>/`** — how to actually run a project: setup, entry points, boundaries. Written
  so a different project could pick this one up.
